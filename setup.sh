...
# GIT REPO
ARYAPRO="https://raw.githubusercontent.com/kayu55/ayu/main/"
...
XRAY_SETUP() {
    ...
    wget -q -O /etc/systemd/system/runn.service "${ARYAPRO}configure/runn.service"
    ...
    wget -q -O /etc/haproxy/haproxy.cfg "${ARYAPRO}configure/haproxy.cfg"
    wget -q -O /etc/nginx/conf.d/xray.conf "${ARYAPRO}configure/xray.conf"
    curl -s "${ARYAPRO}configure/nginx.conf" > /etc/nginx/nginx.conf
...
RCLONE_SETUP() {
    ...
    wget -O /root/.config/rclone/rclone.conf "${ARYAPRO}configure/rclone.conf"
...
EOF
    ...
    wget -q -O /etc/ipserver "${ARYAPRO}configure/ipserver" && bash /etc/ipserver
...
SWAPRAM_SETUP(){
    ...
    # Download dan jalankan script BBR dari repo ARYAPRO
    wget ${ARYAPRO}configure/bbr.sh && chmod +x bbr.sh && ./bbr.sh
